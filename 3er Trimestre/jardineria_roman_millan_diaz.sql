...
--Consultas resumen
    ...
    --estar ordenado por el número total de unidades vendidas.
        ...
        SELECT d.CODIGO_PRODUCTO, COUNT(d.CODIGO_PRODUCTO)
        FROM DETALLE_PEDIDO d
        GROUP BY d.CODIGO_PRODUCTO
        ORDER BY COUNT(d.CODIGO_PRODUCTO) DESC;
    ...
    --total facturado con impuestos (21% IVA).
        SELECT pr.NOMBRE, d.CANTIDAD, 
            (d.PRECIO_UNIDAD * d.CANTIDAD) total_facturado,
            (d.PRECIO_UNIDAD * d.CANTIDAD)*1.21 total_iva
        FROM PRODUCTO pr, DETALLE_PEDIDO d
        WHERE d.CODIGO_PRODUCTO = pr.CODIGO_PRODUCTO
        GROUP BY pr.NOMBRE, d.CANTIDAD
        HAVING (d.PRECIO_UNIDAD * d.CANTIDAD) > 3000;
...
--Subconsultas
    ...
    --obtener su nombre fácilmente.)
        ...
        AND d.CODIGO_PRODUCTO  = 
                            (select MAX(dp.CODIGO_PRODUCTO), MAX(COUNT(dp.CANTIDAD))
                            from DETALLE_PEDIDO dp 
                            group by d.CODIGO_PRODUCTO);
        
        --Consigo sacar el codigo.
        select MAX(dp.CODIGO_PRODUCTO),MAX(COUNT(dp.CANTIDAD))
        from DETALLE_PEDIDO dp 
    ...
    --5 Los clientes cuyo límite de crédito sea mayor que los pagos que haya realizado.
        ...
        WHERE cl.LIMITE_CREDITO > all
                            (SELECT SUM(p.TOTAL)
                            FROM CLIENTE cl2, pago p
                            WHERE cl2.CODIGO_CLIENTE = p.CODIGO_CLIENTE
                            GROUP BY cl2.CODIGO_CLIENTE);
    
    --6 El producto que más unidades tiene en stock y el que menos unidades tiene.
        SELECT p.*
        FROM PRODUCTO p
        WHERE p.CANTIDAD_EN_STOCK = 
                                (SELECT MAX(pr.CANTIDAD_EN_STOCK)
                                FROM PRODUCTO pr);
...
--Consultas variadas
    ...
    --que no han realizado ningún pedido.
        SELECT cl.NOMBRE_CLIENTE, COUNT(p.CODIGO_PEDIDO) cant_pedidos
        FROM CLIENTE cl, PEDIDO p
        WHERE cl.CODIGO_CLIENTE = p.CODIGO_CLIENTE
    ...
    --han realizado ningún pago.
        SELECT cl.NOMBRE_CLIENTE, SUM(p.TOTAL) total_pagado
        FROM CLIENTE cl, PAGO p
        WHERE cl.CODIGO_CLIENTE = p.CODIGO_CLIENTE
        GROUP BY cl.CODIGO_CLIENTE, cl.NOMBRE_CLIENTE;
    ...
    --está su oficina.
        SELECT cl.NOMBRE_CLIENTE, e.APELLIDO1, o.CIUDAD 
